...
- Cho phép kéo để quan sát; nhấp sticker để xoay mặt; `Shift + nhấp` để đảo chiều.
- Hỗ trợ các phím `R`, `L`, `U`, `D`, `F`, `B`; giữ `Shift` để xoay ngược.
...
- Luôn giữ cross (dấu `+`) trắng hoàn chỉnh ở mặt dưới trong chế độ luyện F2L.
- Xáo trộn F2L chỉ được dùng các nước `R`, `L`, `U`, `F`, `B` và các biến thể đảo chiều/180°; không dùng `D`, để cross trắng ở đáy không bị phá vỡ.
